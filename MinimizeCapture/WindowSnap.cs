...
namespace KLCProxy
{
    ...
    class WindowSnap {
        ...
        private WindowSnap(IntPtr hWnd, bool specialCapturing) {
            ...
            GetWindowThreadProcessId(hWnd, out this.procID);
            this.processName = Process.GetProcessById((int)this.procID).ProcessName;

            StringBuilder sb_title = new StringBuilder(1024);
            int length = GetWindowText(hWnd, sb_title, sb_title.Capacity);
            this.windowTitle = sb_title.ToString();
        }

        public void Capture(bool forceSpec=false) {
            bool needSpec = NeedSpecialCapturing(hWnd);
            if (needSpec || forceSpec)
        ...
        public override string ToString() {
            ...
            str.AppendFormat("{0}", this.WindowTitle);
